...
plot.dist.validation.var <- function(simdata,ssize,varfreq){
  ...
  sim_dens <- ggplot(dtemp) + geom_histogram(aes(x=v1.out,y=(..count..)/sum(..count..)),bins=20)
  ...
  ggplot() +
    ...
    geom_density(data=dtemp,aes(x=v1.out.hyper,y=..density..*bw),adjust=5,colour="light blue",size=1) +
    geom_density(data=dtemp,aes(x=v1.out.binom,y=..density..*bw),adjust=5,colour="blue",size=1) +
    geom_vline(xintercept = varfreq, linetype="dashed", color="black", size=0.5) +
...
plot.ss.logistic <- function(df,c_ratio,c_label,t0,r){
  ...
  prev_breaks <- sapply(seq(0,40,10),calc.freq.logistic,t0,r)
  prev_labels <- sprintf("%.4f", prev_breaks)
  
  ggplot(df,aes(x=t,y=n,fill=!!f)) +
    geom_tile() +
    geom_contour(aes(z = !!f), color='black', size=0.4, alpha=0.5, breaks=c(0.5,0.95)) +
    ...
    scale_x_continuous(limits=c(-1,41),expand = c(0,0),
                       sec.axis = sec_axis(trans = ~ calc.freq.logistic(.,t0,r),
