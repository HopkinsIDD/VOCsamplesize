...
p <- c(0.1,seq(0.2,1,0.2))
b <- expand.grid(p,c_ratio)
colnames(b) <- c("p_v1","c_ratio")
mbias <- apply(b,1,function(x) calc.expected.mbias(x["p_v1"][[1]],x["c_ratio"][[1]]))
...
bias_plot <- ggplot(data = df, aes(x=c_ratio, y=mbias)) +
  geom_line(aes(color=factor(p_v1)),size=1) +
...
b <- expand.grid(p,c_ratio)
colnames(b) <- c("p_v1","c_ratio")
n <- apply(b,1,function(x) calc.samplesize.detect(x["p_v1"][[1]],prob_detect,x["c_ratio"][[1]]))
...
b <- expand.grid(p,c_ratio)
colnames(b) <- c("p_v1","c_ratio")
n <- apply(b,1,function(x) calc.samplesize.prev(x["p_v1"][[1]],conf_measure,precision,x["c_ratio"][[1]]))
...
for (c in c_ratios){
  prob <- apply(b,1,function(x) calc.prob.detect.cont(x["n"][[1]],x["t"][[1]],t0,r,c))
...
for (c in c_ratios){
  g <- sapply(t,calc.freq.logistic,t0,r,c)
  df <- data.frame(df,g)
}
