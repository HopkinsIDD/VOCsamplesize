...
mbias <- apply(b,1,function(x) phylosamp::calc_expected_mbias(x["p_v1"][[1]],x["c_ratio"][[1]]))
...
for (c in c_ratios){
  prob <- apply(b,1,function(x) phylosamp::calc_prob_detect_cont(x["n"][[1]],x["t"][[1]],t0,r,c))
...
for (c in c_ratios){
  g <- sapply(t,phylosamp::calc_freq_logistic,t0,r,c)
  df <- data.frame(df,g)
}
